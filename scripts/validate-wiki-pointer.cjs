...
if (pointer !== expected) {
  throw new Error(`docs/wiki.md must contain only ${expected.trim()}`);
}
const gitmodulesPath = path.join(root, '.gitmodules');
if (fs.existsSync(gitmodulesPath)) {
  const gitmodules = fs.readFileSync(gitmodulesPath, 'utf8');
  if (/path\s*=\s*docs\/wiki/.test(gitmodules) || new RegExp(`${repo}\\.wiki\\.git`).test(gitmodules)) {
    throw new Error('docs/wiki must not be configured as a wiki submodule');
  }
}
